...
bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile(const std::string &filename)
{
    ...
    stringstream ss;
    ss << s; // 将s写入ss
    ss >> m_k; // 从左向右依次抽取ss的信息，本次抽取k（节点数）
    ss >> m_L; // 抽取L(层数)
    int n1, n2;
    ss >> n1; // 抽取ScoringType
    ss >> n2; // 抽取WeightingType
    ...
    while(!f.eof())
    {
        ...
        int pid ;
        ssnode >> pid; // 每一行的第一个参数表示：pid
        m_nodes[nid].parent = pid;
        m_nodes[pid].children.push_back(nid);

        int nIsLeaf;
        ssnode >> nIsLeaf; // 每一行的第二个参数表示：nIsLeaf

        stringstream ssd;
        for(int iD=0;iD<F::L;iD++)
        {
            string sElement;
            ssnode >> sElement; // 从第三个参数开始，依次提取参数作为sElement
            ssd << sElement << " ";
      	}
